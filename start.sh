...
mkdir -p /workspaces/.vscode /workspaces/.docker ~/.ssh ~/.logs
...
devcontainer exec --workspace-folder=./code bash -c 'ln -fs /workspaces/.vscode $HOME/ && ln -fs /tmp/.ssh $HOME/'
devcontainer exec --workspace-folder=./code bash -c 'chown $(whoami) -R $HOME/.ssh'
devcontainer exec --workspace-folder=./code code serve-web --host 0.0.0.0 $@ --server-data-dir /workspaces/.vscode/server --without-connection-token --accept-server-license-terms > ~/.logs/vscode-web.log 2>&1 &
